	...
	if " " in element_list[0]:
		...
		for token in terms:
			if token in stop_words:
				...
				last_doc = -1
				a_list = []					
				for (doc,pos) in cur.execute("SELECT doc,position FROM posting  WHERE word = ? order by doc ASC, position ASC",(token,)):
		...
		if a_word in stop_words:
			...
			result = []
			for (doc,) in  cur.execute("SELECT distinct doc FROM posting WHERE word = ?",(a_word,)):
				result.append(doc)
	return result
'''
...
def main():
	global path,connection,wnl,stop_words
	
	#check argument
	if not checkArgv():
		sys.exit("Arrgument error, Please input argument in the follwoing format:\n./boolean_query index_file_location 'query'(query has to be enclosed by '')")
	path = sys.argv[1]
	query = sys.argv[2]
	...
	#connect to database
	connection = connectionDataBase(path) #connected to database file
	...
	if result!=None:
		...
		result = []
	print("Here are the documents satisfy the query",result)
